...
class BA_Block(nn.Module):
    def __init__(self, dim):
        ...
        self.conv_1_h = nn.Conv2d(dim, midplanes, kernel_size=(3, 1), padding=(1, 0), bias=False)
        self.conv_1_w = nn.Conv2d(dim, midplanes, kernel_size=(1, 3), padding=(0, 1), bias=False)

        self.pool_3_h = nn.AdaptiveAvgPool2d((None, 3))
        self.pool_3_w = nn.AdaptiveAvgPool2d((3, None))
        self.conv_3 = nn.Conv2d(dim, midplanes, kernel_size=3, padding=1, bias=False)

        self.pool_5_h = nn.AdaptiveAvgPool2d((None, 5))
        self.pool_5_w = nn.AdaptiveAvgPool2d((5, None))
        self.conv_5 = nn.Conv2d(dim, midplanes, kernel_size=3, padding=1, bias=False)

        self.pool_7_h = nn.AdaptiveAvgPool2d((None, 7))
        self.pool_7_w = nn.AdaptiveAvgPool2d((7, None))
        self.conv_7 = nn.Conv2d(dim, midplanes, kernel_size=3, padding=1, bias=False)

        self.fuse_conv = nn.Conv2d(midplanes * 4, dim, kernel_size=1, padding=0, bias=False)
        self.relu = nn.ReLU(inplace=False)
        self.conv_final = nn.Conv2d(dim, dim, kernel_size=1, bias=True)

        self.mask_conv_1 = nn.Conv2d(dim, dim, kernel_size=3, padding=1)
        self.mask_relu = nn.ReLU(inplace=False)
        self.mask_conv_2 = nn.Conv2d(dim, dim, kernel_size=3, padding=1)
...
class BAM(nn.Module):
    def __init__(self, dim):
        ...
        # Input conv
        self.input_conv = nn.Conv2d(dim, dim, kernel_size=3, padding=1)
        self.input_relu = nn.LeakyReLU(0.2, True)

        # PDC_1
        self.content_conv_d1 = nn.Conv2d(dim, dilated_dim, kernel_size=3, padding=1, dilation=1)
        self.content_conv_d3 = nn.Conv2d(dim, dilated_dim, kernel_size=3, padding=3, dilation=3)
        self.content_conv_d5 = nn.Conv2d(dim, dilated_dim, kernel_size=3, padding=5, dilation=5)
        # PDC_2
        self.content_conv_d1_2 = nn.Conv2d(dim, dilated_dim, kernel_size=3, padding=1, dilation=1)
        self.content_conv_d3_2= nn.Conv2d(dim, dilated_dim, kernel_size=3, padding=3, dilation=3)
        self.content_conv_d5_2 = nn.Conv2d(dim, dilated_dim, kernel_size=3, padding=5, dilation=5)
        # Bridge of CPDC
        self.fuse_conv_1 = nn.Conv2d(3*dilated_dim, dim, kernel_size=3, padding=1)
        self.fuse_relu = nn.LeakyReLU(0.2, True)
        # Fuse BA and CPDC
        self.final_conv = nn.Conv2d(5*dilated_dim, dim, kernel_size=3, padding=1)
        self.fina_relu = nn.LeakyReLU(0.2, True)

        # Blur-aware Attention
        self.BA = BA_Block(dim)
...
class BANet_Plus_model(nn.Module):
    def __init__(self):
        super(BANet_Plus_model, self).__init__()
        ...
        self.BAM_1 = BAM(dim_2)
        self.BAM_2 = BAM(dim_2)
        self.BAM_3 = BAM(dim_2)
        self.BAM_4 = BAM(dim_2)
        self.BAM_5 = BAM(dim_2)
        self.BAM_6 = BAM(dim_2)
        self.BAM_7 = BAM(dim_2)
        self.BAM_8 = BAM(dim_2)
        self.BAM_9 = BAM(dim_2)
        self.BAM_10 = BAM(dim_2)
